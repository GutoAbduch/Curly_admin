...
export default function Appointments() {
  ...
  const [form, setForm] = useState({
    clientName: '',
    clientPhone: '',
    clientEmail: '', // Novo campo opcional
    serviceId: '',
    serviceName: '',
    price: 0,
    barberId: '',
  ...
  const handleServiceSelect = (e) => {
      ...
      if(service) {
          ...
          // Reset caso deselecione
          setForm(prev => ({ ...prev, serviceId: '', serviceName: '', price: 0, duration: 30 }));
      }
  };
  ...
  const checkConflict = (newStart, newDuration, barberId) => {
  ...
  const handleCreateAppointment = async (e) => {
      ...
      // Validação Extra
      if(!form.clientName || !form.clientPhone || !form.serviceId || !form.barberId) {
          alert("Preencha todos os campos obrigatórios.");
          return;
      }

      const conflict = checkConflict(form.time, form.duration, form.barberId);
      
      if (conflict) {
          const canForceFit = ['Admin', 'Gerente', 'Barbeiro'].includes(role);
          if (!canForceFit) {
              alert("⚠️ Horário Indisponível. Por favor, escolha outro horário ou barbeiro.");
              return; 
          }
          const confirmEncaixe = window.confirm(
              `⚠️ ATENÇÃO: ${form.barberName} já tem um cliente às ${conflict.time}.\nDeseja realizar um ENCAIXE?`
          );
      ...
      try {
          ...
          setForm({ 
            ...
            barberId: '', barberName: '', time: '09:00', duration: 30 
          });
      } catch (error) {
  ...
  const handleStatusChange = async (appt, newStatus) => {
      ...
      try {
          await updateDoc(doc(db, `artifacts/${APP_ID}/public/data/appointments`, appt.id), {
              status: newStatus
          });
          loadData();
      } catch (error) {
  ...
  // --- LÓGICA DE FINALIZAÇÃO (CHECKOUT) ---
  const initiateFinish = (appt) => {
      setFinishingAppt(appt);
      setPaymentMethod(''); // Reseta método
      setTip(0); // Reseta gorjeta
      setChangeFor(''); // Reseta troco
      setConsumedSupplies([]); 
      setShowFinishModal(true);
  };

  // Funções de Insumos Manuais (Mantidas)
  ...
  const confirmFinish = async () => {
      ...
      try {
        await runTransaction(db, async (transaction) => {
            ...
            // 2. Financeiro (Lança o valor total recebido)
            ...
            consumedSupplies.forEach(item => {
                if(item.productId && item.qty > 0) {
                    ...
                    transaction.update(productRef, {
                        qty: increment(-parseFloat(item.qty))
                    });
        ...
        loadData();
        alert('Pagamento confirmado e serviço finalizado!');
      } catch (error) {
          console.error(error);
    ...
    <div className="animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        ...
        <div className="flex items-center gap-4 bg-[#111] p-2 rounded-xl border border-[#222]">
            ...
            />
            <button onClick={() => setShowAddModal(true)} className="bg-gold text-black px-4 py-2 rounded-lg font-bold text-sm hover:bg-white transition shadow-[0_0_10px_rgba(212,175,55,0.2)]">
      ...
      {/* MODAL NOVO AGENDAMENTO (ATUALIZADO) */}
      {showAddModal && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-[#111] w-full max-w-md rounded-2xl border border-[#333] p-6 shadow-2xl">
                  ...
                  <form onSubmit={handleCreateAppointment} className="space-y-4">
                      ...
                      {/* DADOS DO CLIENTE */}
                      <div className="space-y-3 p-4 bg-[#0a0a0a] rounded-xl border border-[#222]">
                          <p className="text-[10px] text-[#666] font-bold uppercase">Dados do Cliente</p>
                          <input required placeholder="Nome do Cliente (Obrigatório)" className="input-field" value={form.clientName} onChange={e => setForm({...form, clientName: e.target.value})} />
                          <div className="grid grid-cols-2 gap-2">
                             <input required placeholder="Telefone (Obrigatório)" className="input-field" value={form.clientPhone} onChange={e => setForm({...form, clientPhone: e.target.value})} />
                      ...
                      <div>
                          <label className="text-[10px] text-[#666] font-bold uppercase block mb-1">Horário</label>
                          <input type="time" required className="input-field text-center font-bold text-lg" value={form.time} onChange={e => setForm({...form, time: e.target.value})} />
                      </div>
          ...
          <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-[#111] w-full max-w-lg rounded-2xl border border-[#333] p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="text-center mb-6 border-b border-[#222] pb-6">
                    <p className="text-xs text-[#666] uppercase tracking-widest mb-1">Finalizando Atendimento</p>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-2">
                        {['Dinheiro', 'PIX', 'Débito', 'Crédito'].map(method => (
                            <button 
                                ...
                                className={`py-3 rounded text-xs font-bold border transition-all ${paymentMethod === method ? 'bg-gold text-black border-gold shadow-[0_0_10px_rgba(212,175,55,0.3)]' : 'bg-[#1a1a1a] text-[#666] border-[#333] hover:border-[#666]'}`}
                            >
                                {method === 'Dinheiro' && <i className="fas fa-money-bill-wave mr-2"></i>}
                                {method === 'PIX' && <i className="fas fa-qrcode mr-2"></i>}
                                {method === 'Débito' && <i className="fas fa-credit-card mr-2"></i>}
                                {method === 'Crédito' && <i className="far fa-credit-card mr-2"></i>}
                                {method}
                            </button>
                        ))}
                    </div>

                    {/* Lógica de Troco (Só aparece se for dinheiro) */}
                    {paymentMethod === 'Dinheiro' && (
                        <div className="bg-[#1a1a1a] p-3 rounded border border-[#333] animate-fade-in mt-2">
                            <div className="flex items-center gap-4">
                                <div className="flex-1">
                                    <label className="text-[10px] text-[#666] font-bold uppercase">Troco para quanto?</label>
                                    <input 
                                        type="number" 
                                        placeholder="R$ 0,00" 
                                        className="w-full bg-[#000] border border-[#333] rounded p-2 text-white outline-none focus:border-gold"
                                        value={changeFor}
                                        onChange={e => setChangeFor(e.target.value)}
                                    />
                                </div>
                                <div className="text-right">
                                    <p className="text-[10px] text-[#666] font-bold uppercase">Valor do Troco</p>
                                    <p className={`font-bold text-lg ${changeValue < 0 ? 'text-red-500' : 'text-green-500'}`}>
                                        {formatCurrency(changeValue > 0 ? changeValue : 0)}
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                {/* 2. GORJETA (Só aparece após selecionar pagamento) */}
                {paymentMethod && (
                    <div className="mb-6 animate-fade-in">
                        <label className="block text-xs font-bold text-[#888] uppercase mb-2">Gorjeta (Opcional)</label>
                ...
                <div className="bg-[#050505] p-4 rounded-xl border border-[#222] mb-6">
                    <div className="flex justify-between text-sm mb-1">
                        <span className="text-[#666]">Valor do Serviço</span>
                        <span className="text-[#eee]">{formatCurrency(finishingAppt.price)}</span>
                    </div>
                    {tip > 0 && (
                    ...
                    <div className="flex justify-between text-lg font-black text-gold border-t border-[#222] pt-2 mt-2">
                        <span>TOTAL A PAGAR</span>
                        <span>{formatCurrency(totalToPay)}</span>
                    </div>
                </div>

                {/* 4. INSUMOS (MANUAL) - Colapsável ou discreto */}
                <div className="mb-6 border-t border-[#222] pt-4">
                     <div className="flex justify-between items-end mb-2">
                        <h4 className="text-xs font-bold text-[#666] uppercase">Insumos Gastos (Interno)</h4>
                        <select className="bg-[#000] text-xs text-white border border-[#333] rounded p-1 w-36" onChange={(e) => handleAddSupplyManual(e.target.value)} value="">
                            <option value="">+ Add Manual</option>
                    ...
                    {consumedSupplies.length > 0 && (
                        <div className="bg-[#0a0a0a] p-2 rounded-xl border border-[#333] max-h-32 overflow-y-auto">
                            {consumedSupplies.map((item, idx) => (
                                <div key={idx} className="flex justify-between items-center mb-1 pb-1 border-b border-[#222] last:border-0 last:pb-0">
                                    <span className="text-xs text-[#ccc]">{item.name}</span>
                                    <div className="flex items-center gap-1">
                                        <input type="number" step="0.0001" className="w-14 text-center border border-[#333] rounded p-0.5 text-xs bg-black text-white" value={item.qty} onChange={(e) => handleSupplyChange(idx, e.target.value)} />
                                        <span className="text-[10px] text-[#555]">{item.unit}</span>
                                        <i onClick={() => removeSupply(idx)} className="fas fa-times text-red-500 text-xs cursor-pointer ml-1"></i>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={confirmFinish} disabled={!paymentMethod} className="btn-primary flex-1">
