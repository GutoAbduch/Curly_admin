...
import ClientSidebar from '../components/ClientSidebar'; // <--- IMPORTA A SIDEBAR
import { useClientAuth } from '../context/ClientAuthContext'; // <--- IMPORTA O CONTEXTO
...
export default function ClientLayout() {
  ...
  // Pegamos o cliente logado do contexto
  ...
  return (
    <div className="min-h-screen flex font-sans bg-[#F9F7F2] text-[#1a1a1a] transition-all duration-300" 
         style={{ 
             '--primary': primaryColor,
             backgroundImage: 'radial-gradient(circle at 50% 0%, #ffffff 0%, #F9F7F2 80%)'
         }}>
      
      {/* 1. SIDEBAR (Só aparece se o user estiver logado) */}
      <ClientSidebar />

      {/* 2. CONTEÚDO PRINCIPAL 
         Se estiver logado (desktop), adicionamos padding-left (md:pl-64) para o conteúdo não ficar embaixo da sidebar */}
      <div className={`flex-1 flex flex-col relative transition-all duration-300 ${client ? 'md:pl-64' : ''}`}>
          
          {/* HEADER FLUTUANTE */}
          <header className={`fixed top-0 right-0 z-40 px-4 py-3 pointer-events-none transition-all duration-300 w-full flex justify-end`}>
             <div className="flex gap-2 max-w-5xl mx-auto w-full justify-end">
                 
                 {/* Botão Home (Só mostra se NÃO tiver logado, pois logado já tem na sidebar) */}
                 {!client && (
                     <button 
                        onClick={() => navigate('/')}
                 ...
                 {/* Botão Entrar (Só mostra se NÃO tiver logado) */}
                 {!client && (
                     <button 
